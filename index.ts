 ...
 *
 * Connect your OpenClaw AI agent to VK via Callback API
 */
...
const plugin: {
  ...
  register(api: OpenClawPluginApi) {
    ...
    if (!config?.configured) {
      ...
      console.warn("[VK] Set environment variables: VK_ACCESS_TOKEN, VK_GROUP_ID, VK_CONFIRMATION_TOKEN, VK_CALLBACK_SECRET (optional)");
    } else {
      console.log("[VK] Plugin registered");
      console.log(`[VK] Group ID: ${config.groupId}`);
      console.log("[VK] Callback path: /vk/callback");
    }
  },
};
